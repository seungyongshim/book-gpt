...
export async function fetchModels(baseUrl?: string, signal?: AbortSignal): Promise<ModelListResult> {
  const base = baseUrl || (import.meta as any).env?.VITE_OPENAI_BASE_URL || 'http://localhost:4141/v1';
...
let cache: { ts: number; models: string[] } | null = null;
const DEFAULT_TTL = 5 * 60 * 1000; // 5 minutes

export async function getCachedModels(opts?: { force?: boolean; baseUrl?: string; ttlMs?: number; signal?: AbortSignal; fallback?: string[] }): Promise<string[]> {
