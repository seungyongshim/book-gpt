...
export function setTokenCalibrationFactor(f: number) { calibrationFactor = f > 0 ? f : calibrationFactor; }
...
export function estimateTokens(str: string): number {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    else if (/[\u1100-\u11FF\u3130-\u318F\uAC00-\uD7AF\u4E00-\u9FFF\u3400-\u4DBF]/.test(ch)) cjk++;
...
export interface TokenCalibrationRecord { factor: number; samples: number; updatedAt: number; }

/**
 * 새로운 실제/추정 비율(ratio)을 받아 이동 평균으로 calibrationFactor를 조정.
 * ratio = (실제추정토큰) / (현재추정토큰)
 */
export function applyCalibrationSample(ratio: number) {
  if (!isFinite(ratio) || ratio <= 0) return calibrationFactor;
  // 0.5 ~ 1.5 범위 밖 extreme 은 무시 (이상치)
  if (ratio < 0.5 || ratio > 1.5) return calibrationFactor;
...
export function simpleSummarize(text: string, targetChars: number): string {
  if (!text) return '';
  ...
  for (const s of segments) {
    if ((acc + ' ' + s).trim().length > targetChars) break;
    acc = acc ? acc + ' ' + s : s;
  }
  // 최소 확보 실패 시 단순 절단
  if (!acc) return text.slice(0, targetChars);
  return acc;
}

export function summarizeForReference(raw?: string): string {
  if (!raw) return '';
  return simpleSummarize(raw, 300);
}

export function summarizeWorld(setting: Partial<{ premise?: string; timeline?: string; geography?: string; factions?: string; characters?: string; magicOrTech?: string; constraints?: string; styleGuide?: string; }>): string {
  const parts: string[] = [];
  const push = (label: string, v?: string, cap = 250) => { if (v) parts.push(`[${label}] ` + simpleSummarize(v, cap)); };
...
export function simpleCharTokenEstimate(chars: number, factor = 0.9): number {
  if (!chars) return 0;
