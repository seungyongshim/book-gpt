...
export function usePageGeneration() {
    ...
    const [targetChars, setTargetChars] = useState(12000);
    const [promptTokens, setPromptTokens] = useState(0);
    const [completionChars, setCompletionChars] = useState(0);
    const [finalized, setFinalized] = useState(false);
    const pagesStore = usePagesStore();
    const [lastSavedLen, setLastSavedLen] = useState(0);
    const controllerRef = useRef(null);
    const latestLayerRef = useRef(null);
    const baseContinuationRef = useRef('');
    const finalizeAndPersist = useCallback(async (pageId, buffer, promptEstimated) => {
    ...
    const run = useCallback(async (pageId, layer, cfg) => {
        setOutput('');
        setRunning(true);
        setError(null);
        setLastSavedLen(0);
