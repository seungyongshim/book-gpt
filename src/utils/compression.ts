export interface RefSummary { ref: string; summary: string }

export interface CompressRefsOptions {
  summaries: RefSummary[];
  level: 'L1' | 'L3';
}

export function compressReferences(opts: CompressRefsOptions): RefSummary[] {
...
export function compressWorldSummary(world: string, mode: 'world-compact' | 'world-bullet'): string {
  if (!world) return world;
  if (mode === 'world-compact') {
    if (world.length <= 820) return world;
    const segments = world.split(/\n+/).filter(Boolean);
    let acc = '';
    for (const seg of segments) {
      if ((acc + '\n' + seg).trim().length > 800) break;
      acc = acc ? acc + '\n' + seg : seg;
    }
